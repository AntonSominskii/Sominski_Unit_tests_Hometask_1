...
public class CalculatorTest {
    public static void main(String[] args) {
        // Проверка корректного вычисления суммы со скидкой
        assertThat(Calculator.calculatingDiscount(1000.0, 10)).isEqualTo(900.0);
        assertThat(Calculator.calculatingDiscount(500.0, 50)).isEqualTo(250.0);

        // Проверка выбрасывания исключения при отрицательной сумме покупки
        assertThatThrownBy(() -> Calculator.calculatingDiscount(-100.0, 10))
                .isInstanceOf(ArithmeticException.class)
                .hasMessageContaining("Invalid arguments");

        // Проверка выбрасывания исключения при отрицательном проценте скидки
        assertThatThrownBy(() -> Calculator.calculatingDiscount(100.0, -10))
                .isInstanceOf(ArithmeticException.class)
                .hasMessageContaining("Invalid arguments");

        // Проверка выбрасывания исключения при проценте скидки более 100
        assertThatThrownBy(() -> Calculator.calculatingDiscount(100.0, 110))
                .isInstanceOf(ArithmeticException.class)
                .hasMessageContaining("Invalid arguments");
